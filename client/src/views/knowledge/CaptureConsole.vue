...
function connectSSE() {
  ...
  eventSource.addEventListener('job-deleted', (e: MessageEvent) => {
    try {
      const data = JSON.parse(e.data);
      const jobId = String(data.jobId);
      jobs.value = jobs.value.filter((job) => String(job.id) !== jobId);
      if (expandedId.value === jobId) expandedId.value = null;
